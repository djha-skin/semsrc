...
## Description
Configure the object store to use an S3-compatible cloud storage backend. This allows storing object data in the cloud with scalability, durability, and geographic distribution. The backend should support standard S3 operations and provide efficient upload/download for large objects.


## Test Cases
1. **Basic storage and retrieval**: Store an object to S3 and retrieve it successfully
2. **Large object handling**: Objects larger than 5GB should be uploaded using multipart
3. **Authentication**: Backend should handle AWS authentication correctly
4. **Error handling**: Network failures should be handled gracefully with retries
